...
export class AuthService {
  ...
  async register(registerDto: RegisterDto): Promise<{ message: string; email: string }> {
    ...
    if (existingUser) {
      if (existingUser.isEmailVerified) {
        throw new ConflictException('El usuario ya existe y está verificado');
      } else {
        // Usuario existe pero no verificado, permitir reenvío
  ...
  async verifyEmail(token: string): Promise<AuthResponse> {
    const { email, valid } = await this.tokenService.verifyEmailToken(token);
